...
async function run() {
  ...
  for (const file of files) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await fetch('http://localhost:5173/api/kv', {  // Assumes dev server on port 5173
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ key: `${file}-chunk-${i}`, value: { content: chunks[i], vector: data[0].embedding } })
      });
